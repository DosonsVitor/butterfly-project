...
class DBHelper {
  ...
  Future<FutureOr<void>> onCreate(Database db, int version) async {
    ...
    String sql = "CREATE TABLE BOOK (id INTEGER PRIMARY KEY, nome varchar(100), autor varchar(100), sinopse varchar(1000), paginas integer, paginas_lidas integer, status_id integer, cor1 varchar(50), cor2 varchar(50), icon_url varchar(500));";
    await db.execute(sql);

    sql = "CREATE TABLE STATUS (id INTEGER PRIMARY KEY, status_name VARCHAR(100));";
    await db.execute(sql);

    sql = "CREATE TABLE NOTE (id VARCHAR(100) PRIMARY KEY, notation VARCHAR(1000), page INTEGER, book_id INTEGER, type_id INTEGER);";
    await db.execute(sql);
